...
function buildBroncoscopiaMetrics(
  rows: BroncoscopiaRow[],
  ...
  const broncoContatoIds = getUniqueContatoIds(agendadasRows);
  const base_contatos = broncoContatoIds.size;
  const conversao_consulta = getIntersectionSize(broncoContatoIds, consultaContatoIds);
  ...
  agendadasRows.forEach((row) => {
    ...
    porTipoPacienteMap[tipo] = (porTipoPacienteMap[tipo] ?? 0) + 1;
  });

  const por_tipo_paciente = Object.entries(porTipoPacienteMap)
    .map(([name, value]) => ({ name, value }))
  ...
  const porCodigosMap: Record<string, number> = {};
  agendadasRows.forEach((row) => {
    const quantidadeCodigos = getDimensionLabel(row.quantidade_codigos);
    porCodigosMap[quantidadeCodigos] = (porCodigosMap[quantidadeCodigos] ?? 0) + 1;
  });

  const por_codigos = Object.entries(porCodigosMap)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value);
  ...
  const porTipoRealizadoMap: Record<string, number> = {};
  realizadasRows.forEach((row) => {
    const tipo = getDimensionLabel(row.tipo_paciente);
    porTipoRealizadoMap[tipo] = (porTipoRealizadoMap[tipo] ?? 0) + 1;
  });
  const por_tipo_realizado = Object.entries(porTipoRealizadoMap)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value);
  ...
  const funil_registros: FunnelStageDrilldownRecord[] = [...rows]
    ...
    .map((row) => {
      ...
      return {
        ...
        meta: {
          tipoPaciente: getDimensionLabel(row.tipo_paciente),
          modalidade: getDimensionLabel(row.modalidade_pagamento),
...
export function useBroncoscopiaData() {
  ...
  const { data: allRows = [], isLoading: loadingMain } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return (data ?? []) as BroncoscopiaRow[];
    },
    staleTime: 5 * 60 * 1000,
  });
  ...
  const metrics = useMemo(() => {
    ...
    const current = buildBroncoscopiaMetrics(
    ...
    const previous = buildBroncoscopiaMetrics(
    ...
    const comparisons = {
      ...
      charts: {
        ...
        comparativo_tipo_paciente: buildMetricComparison(
          ...
          sumByNumberKey(previous.por_tipo_paciente, "value")
        ),
        por_tipo_paciente: buildMetricComparison(
          sumByNumberKey(current.por_tipo_paciente, "value"),
          sumByNumberKey(previous.por_tipo_paciente, "value")
        ),
        ...
        por_codigos: buildMetricComparison(
          sumByNumberKey(current.por_codigos, "value"),
          sumByNumberKey(previous.por_codigos, "value")
        ),
        evolucao: buildMetricComparison(
          sumByNumberKey(current.evolucao, "value"),
          sumByNumberKey(previous.evolucao, "value")
        ),
        por_tipo_realizado: buildMetricComparison(
          sumByNumberKey(current.por_tipo_realizado, "value"),
          sumByNumberKey(previous.por_tipo_realizado, "value")
        ),
      },
    };
